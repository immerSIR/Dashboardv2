...
| `PATCH·DELETE /MapApi/organisations/<pk>/members/<user_id>/` | Bearer | Modifie (`email, first_name, last_name, phone, org_role`) / retire un membre. **PATCH** = admin d'org / super admin (n'importe quel membre) **ou un agent de bureau de cette org — mais uniquement sur un `field_agent`, et sans pouvoir changer son rôle** *(2026)* (modifier un admin/bureau → `403` ; envoyer `org_role` en tant que bureau → `403`, `field: "org_role"`). **DELETE** = admin d'org / super admin (n'importe quel membre) **ou un agent de bureau de cette org — uniquement un `field_agent`** *(2026)* (supprimer un admin/bureau → `403 "Un agent de bureau ne peut supprimer qu'un agent de terrain."`). **Anti-verrouillage : impossible de retirer/rétrograder le dernier admin actif (`400`) ; un Super Admin peut outrepasser.** Erreurs de validation **structurées par champ** *(2026)* — cf. la création ci-dessous. |
| `POST /MapApi/organisations/<pk>/agents/create/` | Bearer + **admin d'org OU agent de bureau de cette org** (ou super admin) *(2026 — les agents de bureau peuvent désormais créer des agents de terrain, comme ils peuvent les modifier/supprimer ; auparavant réservé à l'admin d'org → `403`)* | Crée un agent de terrain en un appel `{first_name,last_name,email,phone,address?}` → renvoie `initial_pin`, `must_change_pin`, envoie les identifiants par email. (La création de **staff** — admin/bureau — via `…/staff/create/` reste réservée à l'admin d'org.) **Erreurs de validation (`400`) structurées par champ** *(2026)* : `{error: "<résumé>", field: "email"\|"phone"\|"org_role", errors: {<champ>: "<msg>"}}` pour une erreur sur un seul champ (email/téléphone en double), et `{error, fields: ["email", …], errors: {…}}` pour les champs requis manquants — mappe `errors[field]` / `fields` sur tes inputs. Le message lisible `error` reste présent. |
...
| `GET /MapApi/impact/incidents/` **(Bearer — Super Admin OU org_admin/bureau)** *(2026)* | **Liste paginée + cherchable des incidents derrière le tableau de bord impact** — mêmes portée & filtres que `/impact/` (Super Admin = toutes orgs ; org_admin/bureau = incidents pris en charge par SON org). `?status=…`, période `?filter_type=…`, **`?search=`** (titre/description/zone), `?page=&page_size=`. Renvoie `{count,next,previous,results}` ; chaque résultat = l'incident (`id,title,zone,description,lattitude,longitude,severity,etat,take_in_charge_mode,photo,thumbnail,created_at,resolution_*`) **+ `prediction`** (null si aucune — sinon `{incident_type,macro_category,sub_category,global_impact_score,impact_radius_meters,spread_vectors,direct{…},indirect{…,vigilance_radius_meters},infrastructure{par type},indirect_infrastructure}`), **`tasks`** (toutes les tâches : `{id,title,state,is_confirmed,assigned_to,assigned_to_name,start_date,end_date}`), et **`collaborating_organisations`** (`[{id,name,relation}]` — leader/assigned/collaborator). |
| `GET /MapApi/impact/` **(Bearer — Super Admin OU org_admin/bureau)** *(2026)* | **Tableau de bord IMPACT** — bilan consolidé des incidents **résolus** et/ou **pris en compte avec ≥1 tâche effectuée**. Alimenté par les analyses IA. **Portée par rôle** *(2026)* : le **Super Admin** voit **toutes les organisations** (plateforme) **avec les sections plateforme** ; un **admin d'org ou agent de bureau** ne voit que **l'impact de SON org** (incidents pris en charge par son org, `taken_by`∈org) et les **sections plateforme sont omises** (pas de `resolution`/`mobilization`/`citizen_contribution`). Agents de terrain / autres → `403`. `filters.scope` = `"platform"` ou `"organisation"` (+ `filters.organisation`). **Filtres :** `?status=all\|resolved\|taken_action` (défaut `all`) et période `?filter_type=…` (même enum que `incident-filter`, sur `created_at` ; `custom_range`→`custom_start`/`custom_end`). **Toujours renvoyé :** `beneficiaries.{direct,indirect}.{total,men,women,children}` (directs = personnes exposées, indirects = population potentielle) ; `infrastructure_protected.{total, by_type{schools,health_centers,water_points,markets,maternities,nurseries,main_roads_bridges,residential_buildings}, indirect_by_type}` (filtrable par type côté front) ; **`cumulative_impact_area_ha`** = Σ(π·`impact_radius_meters`²)/10 000. **Sections réservées au Super Admin :** `resolution.{avg_resolution_days, resolution_rate{resolved,reported,rate,percentage}}` ; `mobilization.{organisations_involved, field_agents_mobilized, collaborations_created, incidents_collaborative, incidents_individual}` ; `citizen_contribution.{reports_received, reports_verified (= incidents au-delà de `declared`), reports_led_to_action (= ≥1 tâche effectuée), active_citizen_contributors}`. |
...
| `GET·PATCH /MapApi/notifications/<id>/` *(2026)* | Bearer | **GET** = détail d'une notification (les siennes uniquement → sinon `404`). **PATCH** = marquer lue/non lue au clic : corps `{read: true}` (seul `read` est modifiable ; défaut `true` si omis) → renvoie la notification mise à jour. Limité aux notifications de l'utilisateur connecté (celle d'un autre → `404`). |
| `POST /MapApi/notifications/mark-all-read/` *(2026)* | Bearer | Marque **toutes** les notifications non lues de l'utilisateur comme lues → `{marked_read: <n>}`. |
...
| Chemin WS | Push |
|---|---|
| `wss://<api>/ws/notifications/` | `{event:'notification', id, type, title, message, incident_title, read, colaboration, incident, link, created_at}` — notifications de l'utilisateur connecté en temps réel. **`type`** *(2026)* = catégorie (`collaboration_request`, `collaboration_accepted`, `collaboration_declined`, `deadline_warning`, `incident_report`, `incident_assignment`) pour icône/logique ; **`title`** *(2026)* = libellé FR prêt à afficher (**à utiliser au lieu d'un titre en dur**) ; **`incident_title`** = nom de l'incident. **`link`** = cible de redirection au clic, ou `null` *(màj 2026)* : une **`collaboration_request`** (demande reçue par le leader) → `{type:'collaboration_request', tab:'requests', collaboration_id, incident_id, url:'/collaboration?tab=requests'}` (l'onglet **Demandes** où l'on accepte/refuse — **pas** le détail) ; toute **autre** notif de collaboration → `{type:'collaboration', tab:'mes-collaborations', collaboration_id, incident_id, url:'/collaboration-detail/<collaboration_id>'}` (utilise désormais le **collaboration_id**, plus l'incident_id) ; une notif d'incident seul → `{type:'incident', incident_id, url:'/incidents/<id>'}`. Les mêmes champs `type`/`title`/`incident_title`/`link` sont sur le REST `GET /notifications/`. |
