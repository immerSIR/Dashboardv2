...
| `GET /MapApi/impact/` **(Bearer + Super Admin)** *(2026)* | **Tableau de bord IMPACT** — bilan consolidé des incidents **résolus** et/ou **pris en compte avec ≥1 tâche effectuée**. Alimenté par les analyses IA. **Filtres :** `?status=all\|resolved\|taken_action` (défaut `all`) et période `?filter_type=…` (même enum que `incident-filter`, sur `created_at` ; `custom_range`→`custom_start`/`custom_end`). **Renvoie :** `beneficiaries.{direct,indirect}.{total,men,women,children}` (directs = personnes exposées, indirects = population potentielle) ; `infrastructure_protected.{total, by_type{schools,health_centers,water_points,markets,maternities,nurseries,main_roads_bridges,residential_buildings}, indirect_by_type}` (filtrable par type côté front) ; `resolution.{avg_resolution_days, resolution_rate{resolved,reported,rate,percentage}}` ; `mobilization.{organisations_involved, field_agents_mobilized, collaborations_created, incidents_collaborative, incidents_individual}` ; `citizen_contribution.{reports_received, reports_verified (= incidents au-delà de `declared`), reports_led_to_action (= ≥1 tâche effectuée), active_citizen_contributors}` ; **`cumulative_impact_area_ha`** = Σ(π·`impact_radius_meters`²)/10 000. |
