...
| `GET /MapApi/org-incidents/` | Bearer | **« Mes interventions »** — incidents que l'org de l'appelant a pris en charge **EN INTERNE** (`take_in_charge_mode='internal'` **ET `taken_by` ∈ l'org de l'appelant**). *(2026 : désormais limité à l'org de l'appelant — auparavant tous les incidents internes de toutes les orgs fuitaient.)* **Les collaborations sont exclues** (mode `collaborative` / dirigées par une autre org → voir « Mes collaborations »). `?source=agents|citizens|all` (défaut `all`) optionnel : sous-filtre par **qui a signalé** l'incident (agents de terrain de l'org vs autres). Paginé (20/page). |
