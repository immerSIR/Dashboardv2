...
| Chemin WS | Push |
|---|---|
| `wss://<api>/ws/notifications/` | `{event:'notification', id, type, title, message, incident_title, read, colaboration, incident, link, created_at}` — notifications de l'utilisateur connecté en temps réel. **`type`** *(2026)* = catégorie (`collaboration_request`, `collaboration_accepted`, `collaboration_declined`, `deadline_warning`, `incident_report`, `incident_assignment`) pour icône/logique ; **`title`** *(2026)* = libellé FR prêt à afficher (**à utiliser au lieu d'un titre en dur**) ; **`incident_title`** = nom de l'incident. **`link`** = cible de redirection au clic, ou `null` *(màj 2026)* : une **`collaboration_request`** (demande reçue par le leader) → `{type:'collaboration_request', tab:'demandes', collaboration_id, incident_id, url:'/collaboration?tab=demandes'}` (l'onglet **Demandes** où l'on accepte/refuse — **pas** le détail) ; toute **autre** notif de collaboration → `{type:'collaboration', tab:'mes-collaborations', collaboration_id, incident_id, url:'/collaboration-detail/<collaboration_id>'}` (utilise désormais le **collaboration_id**, plus l'incident_id) ; une notif d'incident seul → `{type:'incident', incident_id, url:'/incidents/<id>'}`. Les mêmes champs `type`/`title`/`incident_title`/`link` sont sur le REST `GET /notifications/`. |
