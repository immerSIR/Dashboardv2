...
| `PATCH·DELETE /MapApi/organisations/<pk>/members/<user_id>/` | Bearer + org_admin/bureau ou staff | Modifie (`email, first_name, last_name, phone, org_role`) / retire un membre. **Anti-verrouillage : un admin d'org ne peut pas retirer/rétrograder le dernier admin actif (`400`) ; un Super Admin peut outrepasser.** |
...
| Méthode · Chemin | Auth | Notes |
|---|---|---|
| `GET·POST /MapApi/collaboration/` | Bearer | Liste. **`?scope=mine\|received\|all`** *(2026, défaut `all`)* : **`mine`** = mes propres collaborations (onglet « Mes collaborations » — une carte par incident avec **mon** rôle) ; **`received`** = demandes faites par d'autres sur les incidents que **je dirige** (onglet « Demandes ») ; **`all`** = les deux. Autres filtres `?status=&role=&incident_id=`. POST `{incident, role(contributor\|observer), motivation?, end_date?}` — `leader` rejeté ; doublon → `400`. **Ne PAS envoyer `user` — il est forcé côté serveur à l'utilisateur authentifié (lecture seule) ; envoyer un mauvais `user` provoquait `400 Invalid pk`.** |
...
| `GET /MapApi/collaborations/dashboard/` | Bearer | Liste enrichie. **`?scope=mine\|received\|all`** *(2026, défaut `all`)* — même sens que sur `/collaboration/` (`mine` pour « Mes collaborations », `received` pour « Demandes »). Filtres `?status=all\|in-progress\|completed\|pending\|accepted\|declined`, `?date_from=&date_to=&search=`. |
...
| `GET /MapApi/activity-feed/` *(2026)* | Bearer | Activité de la plateforme **hors organisation de l'utilisateur** (prises en charge / résolutions d'incidents, etc.), plus récente d'abord, paginée. Éléments : `{action, user_name, organisation_name, created_at, timeStamp}`. (Sourcé depuis `UserAction` : actions sur incidents **+ demandes/acceptations/refus de collaboration**. Rapports/IA encore à ajouter.) |
