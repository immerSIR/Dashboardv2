...
| `GET /MapApi/activity-feed/` *(2026)* | Bearer | Activité de la plateforme **hors organisation de l'utilisateur**, plus récente d'abord, paginée. Éléments : `{id, action, user, user_name, organisation_name, created_at, timeStamp}`. **`action`** est une phrase verbale **en français, sans l'acteur et avec le titre de l'incident** *(2026 — certaines étaient en anglais et affichaient l'UUID brut)*, ex. *« a pris en charge l'incident «…» en mode interne. »* — à afficher comme `{user_name ou organisation_name} {action}`. Sourcé depuis `UserAction`. **Temps réel :** aussi poussé via `wss://<api>/ws/activity-feed/` *(2026)* — voir la table WebSocket. |
...
| `wss://<api>/ws/activity-feed/` *(2026)* | `{event:'activity', id, action, user, user_name, organisation_name, organisation_id, created_at, timeStamp}` — flux d'activité de la plateforme en temps réel. **Filtré côté serveur comme le REST `GET /activity-feed/`** : chaque utilisateur ne reçoit que l'activité des **autres** organisations (celle de sa propre org est exclue). Ajoute les nouveaux éléments en tête du flux à la réception. |
