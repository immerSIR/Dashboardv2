...
| `GET /MapApi/collaborations/dashboard/` | Bearer | Liste enrichie. **`?scope=self\|received\|all`** *(2026, défaut `all`)* — même sens que sur `/collaboration/` (`self` pour « Mes collaborations », `received` pour « Demandes »). Filtres `?status=all\|in-progress\|completed\|pending\|accepted\|declined`, `?date_from=&date_to=&search=`. |
