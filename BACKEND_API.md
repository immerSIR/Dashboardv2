...
| `GET /MapApi/org-incidents/` | Bearer | **"Mes interventions"** — incidents the caller's org took in charge **internally** (`take_in_charge_mode='internal'` **AND `taken_by` ∈ the caller's org**). *(2026: now scoped to the caller's org — previously leaked every org's internal incidents.)* **Collaborations are excluded** (those are `collaborative`/led by another org → see "Mes collaborations"). Optional `?source=agents|citizens|all` (default `all`) sub-filters by **who reported** the incident (org's field agents vs others). Paginated (20/page). |
