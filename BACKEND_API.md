...
| `GET /MapApi/collaborations/dashboard/` | Bearer | Enriched list. **`?scope=self\|received\|all`** *(2026, default `all`)* — same meaning as on `/collaboration/` (use `self` for "Mes collaborations", `received` for "Demandes"). Filters `?status=all\|in-progress\|completed\|pending\|accepted\|declined`, `?date_from=&date_to=&search=`. |
